...
let pos_to_string ((startline, startcol, endline, endcol) : pos) (range : bool)
    : string =
  if range then
    Printf.sprintf "line %d, col %d--line %d, col %d" startline startcol endline
      endcol
  else
      ...
      (fun ((toks : pos tok list), (line : int), (col : int))
        ...
        | Text t -> (
            if t = "true" then
              ...
              let tLen = String.length t in
              try
                ( TInt (Int64.of_string t, (line, col, line, col + tLen)) :: toks,
                  line,
                  col + tLen )
              with Failure _ ->
...
let parse_toks (toks : pos tok list) : pos sexp list =
  ...
  (* Takes a token list and produces a single sexp, along with any leftover
     tokens *)
          ...
          raise
            (SexpParseFailure
               (Printf.sprintf "Unmatched left paren at %s"
                  (pos_to_string p1 false) ) ) )
    | RPAREN p :: rest ->
        raise
          (SexpParseFailure
             (Printf.sprintf "Unmatched right paren at %s"
                (pos_to_string p false) ) )
  (* Takes a token list and produces as many sexps as it can, along with any
     leftover tokens *)
