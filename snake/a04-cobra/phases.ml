...
let rec err_concat (fs : 'a fallible list) : 'a list fallible =
  match fs with
  | [] -> Ok []
  | Ok e :: rest -> err_concat rest |> Result.map (List.cons e)
  | Error e :: rest ->
      err_concat rest
      |> Result.fold ~ok:(Fun.const e) ~error:(List.append e)
      |> Result.error
;;
...
let print_trace (trace : phase list) : string list =
  ...
  let string_of_phase p =
    match p with
    | Source s -> s
    | Parsed p | WellFormed p | Desugared p -> string_of_program p
