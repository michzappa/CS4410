...
let deepest_stack e env =
  ...
  and name_to_offset name =
    ...
    | RegOffset (bytes, RBP) ->
        (* negative because stack direction *)
        bytes / (-1 * word_size)
    | _ -> 0
  in
...
let rename (p : tag program) : tag program =
  let rec rename_expr (env : (string * string) list) (e : tag expr) =
    let rename_env = rename_expr env in
    match e with
    ...
    | ELet (binds, body, tag) ->
        let rename_bind (acc_binds, acc_env) (name, binding, tag) =
          ...
          let new_binding = rename_expr acc_env binding in
        ...
        ELet (binds, rename_expr new_env body, tag)
    | EApp (name, args, tag) -> EApp (name, List.map rename_env args, tag)
  and rename_decl (d : tag decl) : tag decl =
    match d with
    | DFun (name, args, body, tag) ->
        let new_args, new_env =
          let rename_arg (name, tag) (acc_args, acc_env) =
            let new_name = sprintf "%s#%d" name tag in
            ((new_name, tag) :: acc_args, (name, new_name) :: acc_env)
          in
          List.fold_right rename_arg args ([], [])
        in
        (* function names are not renamed at the moment *)
        DFun (name, new_args, rename_expr new_env body, tag)
  in
  match p with
  | Program (decls, expr, tag) ->
      let decls_rn = List.map rename_decl decls in
      let expr_rn = rename_expr [] expr in
      Program (decls_rn, expr_rn, tag)
;;
...
let anf (p : tag program) : unit aprogram =
  ...
  and helpC (e : tag expr) : unit cexpr * (string * unit cexpr) list =
    ...
    | EApp (funname, args, _) ->
        let arg_imms, arg_ctx = args |> List.map helpI |> List.split in
        let arg_ctx = List.concat arg_ctx in
        let ans = CApp (funname, arg_imms, ()) in
        (ans, arg_ctx)
    | ENumber _ | EBool _ | EId _ ->
  ...
  and helpI (e : tag expr) : unit immexpr * (string * unit cexpr) list =
    ...
    | EPrim2 (EP2 op, left, right, tag) ->
        ...
        ( ImmId (tmp, ()),
          ...
          @ [(tmp, CEPrim2 (op, left_imm, right_imm, ()))] )
    | EPrim2 (LP2 op, left, right, tag) ->
        let tmp = sprintf "binop_%d" tag in
        let left_imm, left_setup = helpI left in
        let right_anf = helpA right in
        ( ImmId (tmp, ()),
          left_setup @ [(tmp, CLPrim2 (op, left_imm, right_anf, ()))] )
...
let is_well_formed (p : sourcespan program) : sourcespan program fallible =
  ...
  let rec wfE
    ...
    | ELet (bindings, body, _) ->
        ...
        let dup_names_errors =
          ...
          |> List.map name_of_bind
          (* Make an error for each duplicate binding *)
          |> process_duplicate_names
               (fun dupname ->
                 bindings
                 |> List.filter (bind_name_matches dupname)
                 |> List.map info_of_bind )
               duplicate_id
        in
        ...
        let new_name_list, binding_errors =
          List.fold_left
            (fun (names, errs) (name, bind, info) ->
              ((name, (L.Variable, info)) :: names, errs @ wfE names bind) )
            (name_list, []) bindings
        in
  ...
  let wfD
    ...
    | DFun (_, arguments, body, _) ->
        ...
        let arg_name_list =
          List.map (fun (name, info) -> (name, (L.Variable, info))) arguments
        in
  ...
  | Program (decls, body, _) -> (
      ...
      let all_errors = dup_names_errors @ decl_errors @ body_errors in
...
let naive_stack_allocation (prog : tag aprogram) : tag aprogram * arg envt =
  ...
  and allocate_aexpr (expr : tag aexpr) (si : int) : arg envt =
    ...
    | ALet (id, bind, body, _) ->
        let binding_env =
          (id, RegOffset (8 * ~-si, RBP)) :: allocate_cexpr bind (si + 1)
        in
...
(* XXX: think about num_args - I don't think it relates to the
   environment at all, but is used for tail-call optimization. *)
...
let rec compile_aexpr
  ...
  | ALet (name, bind, body, tag) ->
      let let_name = sprintf "let$%s#%d" name tag in
      let bound_value = compile_cexpr bind env caller_args false in
...
and compile_cexpr
  ...
  | CPrim1 (((IsBool | IsNum) as op), x, tag) ->
      let prim1_name = sprintf "prim1$%s#%d" (name_of_op1 op) tag in
...
let compile_prog ((anfed : tag aprogram), (env : arg envt)) : string =
  ...
  | AProgram (decls, expr, _) ->
      ...
      let expr_code =
        match
          compile_decl (ADFun ("our_code_starts_here", [], expr, 0)) env
        with
          ...
          :: insts ->
            let new_setup =
              [ IGlobal "our_code_starts_here";
                ...
                i_rsp_into_rbp;
                i_reserve_locals;
                IMov (Reg (List.nth first_six_args_registers 0), Reg RBP);
