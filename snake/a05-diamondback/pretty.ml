...
let rec string_of_expr_with (print_a : 'a -> string) (e : 'a expr) : string =
  ...
  | ELet (binds, body, a) ->
      let binds_strs =
        List.map (fun (x, e, _) -> sprintf "%s = %s" x (string_of_expr e)) binds
      in
...
let format_expr (e : 'a expr) (print_a : 'a -> string) : string =
  ...
  in
  let open_paren fmt =
    pp_open_box fmt 2; pp_print_string fmt "("; pp_print_cut fmt ()
  in
  let close_paren fmt =
  ...
  let rec help e fmt =
    ...
    | ENumber (n, a) ->
        ...
        close_paren fmt
    | EBool (b, a) ->
        open_label fmt "EBool" a; pp_print_bool fmt b; close_paren fmt
    | EId (x, a) ->
