...
let test_run ?args:(args=[]) ?std_input:(std_input="") program_str outfile expected ?cmp:(cmp=(=)) _test_ctxt =
...
let test_run_anf ?args:(args=[]) ?std_input:(std_input="") program_anf outfile expected ?cmp:(cmp=(=)) _test_ctxt =
  let full_outfile = "output/" ^ outfile in
  let result = run_anf program_anf full_outfile run_no_vg args std_input in
  assert_equal (Ok(expected ^ "\n")) result ~cmp:cmp ~printer:result_printer

let test_run_valgrind ?args:(args=[]) ?std_input:(std_input="") program_str outfile expected ?cmp:(cmp=(=)) _test_ctxt =
  let full_outfile = "output/" ^ outfile in
  let result =
    try
      let program = parse_string outfile program_str in
...
let test_err ?args:(args=[]) ?std_input:(std_input="") program_str outfile errmsg ?vg:(vg=false) _test_ctxt =
  let full_outfile = "output/" ^ outfile in
