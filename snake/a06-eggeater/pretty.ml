...
let rec string_of_aexpr_with (print_a : 'a -> string) (e : 'a aexpr) : string =
  ...
  match e with
  | ALet (Named name, e, b, a) ->
      sprintf "(alet %s = %s in %s)%s" name
        (string_of_cexpr_with print_a e)
        (string_of_aexpr b) (print_a a)
  | ALet (Blank, e, b, a) ->
      sprintf "(alet %s = %s in %s)%s" "_"
        (string_of_cexpr_with print_a e)
        (string_of_aexpr b) (print_a a)
...
and string_of_adecl_with (print_a : 'a -> string) (d : 'a adecl) : string =
  ...
  | ADFun (name, args, body, a) ->
      sprintf "(fun %s(%s): %s)%s" name
        (ExtString.String.join ", " args)
        (string_of_aexpr_with print_a body)
        (print_a a)
;;

let string_of_aexpr (e : 'a aexpr) : string =
  string_of_aexpr_with (fun _ -> "") e
;;

let string_of_cexpr (c : 'a cexpr) : string =
  string_of_cexpr_with (fun _ -> "") c
;;
...
let string_of_adecl (d : 'a adecl) : string =
  string_of_adecl_with (fun _ -> "") d
;;

let string_of_aprogram (p : 'a aprogram) : string =
  string_of_aprogram_with (fun _ -> "") p
;;
