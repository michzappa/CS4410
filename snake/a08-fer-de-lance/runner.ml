...
let compile_file_to_string (name : string) (input_file : string) :
    string pipeline =
...
let test_err
    ...
    ?(vg = false)
    _test_ctxt =
  let _ = should_infer in
  let _ = should_check in
  let full_outfile = "output/" ^ outfile in
...
let test_err_input
    ...
    expected
    test_ctxt =
  test_err ~should_infer ~should_check ~args ~std_input:""
    (string_of_file ("input/" ^ filename))
    filename expected test_ctxt
;;
...
let test_does_err filename test_ctxt =
  ...
  in
  let input = if Sys.file_exists infile then string_of_file infile else "" in
  test_err ~should_infer:opts.infer ~should_check:opts.check ~args
    ~std_input:input prog ("do_err/" ^ filename) err ~vg:opts.valgrind test_ctxt
;;
...
let input_file_test_suite () =
  ...
  >::: [ "do_pass"
         >::: List.map
                ...
                (safe_readdir "input/do_pass" ".fdl");
         "do_err"
         >::: List.map
                (fun f -> f >:: test_does_err f)
                (safe_readdir "input/do_err" ".fdl");
         "dont_pass"
         >::: List.map
                (fun f -> f >:: test_doesnt_run f)
                (safe_readdir "input/dont_pass" ".fdl");
         "dont_err"
         >::: List.map
                (fun f -> f >:: test_doesnt_err f)
                (safe_readdir "input/dont_err" ".fdl") ]
;;
