...
let print_env env how =
  printf "Env is\n";
  List.iter (fun (id, bind) -> printf "  %s -> %s\n" id (how bind)) env
;;
  ...
  let rec helpA e =
    ...
    | ALetRec (binds, body, _) ->
        List.fold_left max (helpA body)
          (List.map
             (fun (name, bind) -> max (name_to_offset name) (helpC bind))
             binds )
...
let reserve (size : int64) (name : string) : instruction list =
  [ ILineComment (sprintf "Reserving space for %s on the heap" name);
    IMov (Reg (native_arg_reg 0), Reg heap_reg);
    IMov (Reg (native_arg_reg 1), Const size);
    IMov (Reg (native_arg_reg 2), Reg RBP);
    IMov (Reg (native_arg_reg 3), Reg RSP);
    ICall (Label "try_gc");
...
let rec compile_fun
  ...
  let stack_setup =
    ...
    let stack_slots = num_param_slots + num_free_vars in
...
and compile_cexpr
  ...
  | CNativeApp (fname, args, tag) ->
      ...
      let args_in_reg_inst =
        List.mapi
          (fun idx arg -> IMov (Reg (native_arg_reg idx), compile_imm arg env))
          args_in_reg
      in
...
let compile_prog ((anfed : tag aprogram), (global_env : arg name_envt tag_envt))
  ...
  | AProgram (expr, tag) ->
      ...
      let prelude =
        ...
        @ [IExtern snake_entry_base_pointer; IExtern snake_heap_reg_save]
      in
      ...
      let expr_code =
        let setup, body, teardown =
          compile_fun "our_code_starts_here" [] [] expr
          ...
          | (ILabel "our_code_starts_here" as i_label)
            ...
            :: rest ->
              [ IGlobal "our_code_starts_here";
                ...
                IMov (Reg heap_reg, Reg (native_arg_reg 0));
