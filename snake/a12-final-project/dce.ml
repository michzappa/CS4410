...
let dead_code_elimination (p : 'p aprogram) : unit aprogram =
  ...
  let helpI (imm : 'imm immexpr) : unit immexpr dce_info =
    ...
    | ImmString (s, _) -> make_info (ImmString (s, ())) true SS.empty
    | ImmROId _ -> ice "ImmString in DCE"
  in
  ...
  and helpC (cexp : 'c cexpr) : unit cexpr dce_info =
    ...
    | CLambda (args, body, _, _) ->
        ...
        { exp=
          ...
          is_pure= true;
          free_vars= new_fv }
    | CROLambda _ -> ice "CROLambda encountered in DCE"
  in
  match p with
  | AProgram (body, read_onlys, _) -> AProgram ((helpA body).exp, read_onlys, ())
;;
