...
let allocating_native_functions : string list =
  ["input"; "str_concat"; "str_slice"; "to_string"; "readline"]
;;
...
let free_vars ?(bound : string list = []) (e : 'a aexpr) : string list =
  ...
  and free_vars_immexpr (e : 'a immexpr) (bound : string list) : string list =
    ...
    | ImmNil _ | ImmNum _ | ImmBool _ | ImmROId _ | ImmString _ -> []
  in
...
let annotate_with_free_vars (prog : tag aprogram) : (tag * freevarst) aprogram =
  ...
  and helpC (cexp : tag cexpr) : (tag * StringSet.t) cexpr =
    ...
    | CROLambda (name, args, body, t) ->
        let annotated_body = helpA body in
        CROLambda (name, args, annotated_body, (t, StringSet.empty))
  and helpI (iexp : tag immexpr) : (tag * StringSet.t) immexpr =
    ...
    | ImmString (_, _) -> ice "ImmString found in alloc"
    | ImmROId (id, r, t) -> ImmROId (id, r, (t, StringSet.empty))
  in
  match prog with
  | AProgram (body, read_onlys, t) ->
      AProgram (helpA body, read_onlys, (t, StringSet.empty))
;;
