...
exception InterpolationExpectedClosingBrace of string * sourcespan

exception InterpolationUnexpectedOpeningBrace of string * sourcespan

exception InterpolationUnexpectedClosingBrace of string * sourcespan

exception InterpolationInvalidIdentifier of string * sourcespan
...
let print_errors (exns : exn list) : string list =
  List.map
    (fun e ->
      ...
      | InterpolationExpectedClosingBrace (str, loc) ->
          sprintf "Expecting closing brace in string '%s' at %s" str
            (string_of_sourcespan loc)
      | InterpolationUnexpectedOpeningBrace (str, loc) ->
          sprintf "Unexpected opening brace in string '%s' at %s" str
            (string_of_sourcespan loc)
      | InterpolationUnexpectedClosingBrace (str, loc) ->
          sprintf "Unexpected closing brace in string '%s' at %s" str
            (string_of_sourcespan loc)
      | InterpolationInvalidIdentifier (id, loc) ->
          sprintf "Non-identifier '%s' found in interpolation at %s" id
            (string_of_sourcespan loc)
