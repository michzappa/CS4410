...
and 'a expr =
  ...
  | EStrConcat of 'a expr * 'a expr * 'a
  | EStrSlice of 'a expr * 'a expr * 'a expr * 'a
  | EStrSliceNoHigh of 'a expr * 'a expr * 'a
...
type 'a immexpr =
  ...
  | ImmId of string * 'a
  | ImmNil of 'a
  (* Exchange EString for ImmString in ANF *)
  | ImmString of string * 'a
  (* Exchange ImmString for ImmRoId in Lifted *)
  | ImmROId of string * read_only_type * 'a
...
and 'a cexpr =
  ...
  (* No free vars but instead has the read only lambda *)
  | CROLambda of string * string list * 'a aexpr * 'a
...
and 'a aprogram = AProgram of 'a aexpr * (read_only_data * string) list * 'a
...
let rec map_tag_E (f : 'a -> 'b) (e : 'a expr) =
  ...
  | EStrConcat (e1, e2, a) -> EStrConcat (map_tag_E f e1, map_tag_E f e2, f a)
  | EStrSlice (e1, e2, e3, a) ->
      EStrSlice (map_tag_E f e1, map_tag_E f e2, map_tag_E f e3, f a)
  | EStrSliceNoHigh (e1, e2, a) ->
      EStrSliceNoHigh (map_tag_E f e1, map_tag_E f e2, f a)
...
and untagE e =
  ...
  | EStrConcat (e1, e2, _) -> EStrConcat (untagE e1, untagE e2, ())
  | EStrSlice (e1, e2, e3, _) -> EStrSlice (untagE e1, untagE e2, untagE e3, ())
  | EStrSliceNoHigh (e1, e2, _) -> EStrSliceNoHigh (untagE e1, untagE e2, ())
  | EGetItem (e, idx, _) -> EGetItem (untagE e, untagE idx, ())
  | ESetItem (e, idx, newval, _) ->
      ESetItem (untagE e, untagE idx, untagE newval, ())
  | EId (x, _) -> EId (x, ())
  | EString (x, _) -> EString (x, ())
...
let map_tag_A (f : 'a -> 'b) (p : 'a aprogram) : 'b aprogram =
  let helpI (i : 'a immexpr) : 'b immexpr =
    ...
    | ImmString (s, a) -> ImmString (s, f a)
    | ImmROId (x, r, a) -> ImmROId (x, r, f a)
  in
  ...
  and helpC (c : 'a cexpr) : 'b cexpr =
    ...
    | CROLambda (name, args, body, a) -> CROLambda (name, args, helpA body, f a)
  in
  let helpP (p : 'a aprogram) : 'b aprogram =
    match p with
    | AProgram (body, read_onlys, a) -> AProgram (helpA body, read_onlys, f a)
  in
  helpP p
;;
...
let rename_and_tag (p : tag program) : tag program =
  ...
  and helpE env e =
    ...
    | EStrConcat (e1, e2, tag) -> EStrConcat (helpE env e1, helpE env e2, tag)
    | EStrSlice (e1, e2, e3, tag) ->
        EStrSlice (helpE env e1, helpE env e2, helpE env e3, tag)
    | EStrSliceNoHigh (e1, e2, tag) ->
        EStrSliceNoHigh (helpE env e1, helpE env e2, tag)
    ...
    | ENumber _ | EBool _ | ENil _ | EString _ -> e
