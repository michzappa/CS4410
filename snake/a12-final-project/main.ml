...
let set_strategy s =
  ...
  | _ ->
      raise
        (Arg.Bad
           (sprintf
              "'%s' is not an allocation strategy, use either naive or register"
              s ) )
;;
...
let () =
  let speclist =
    [ ("-t", Arg.Set show_trace, "Display the trace of compilation");
      ...
      ("-no-dce", Arg.Set no_dce, "Disable the dead code elimination pass");
  ...
  let sep = "\n=================\n" in
  match
    compile_file_to_string ~no_builtins:!no_builtins ~no_dce:!no_dce
      !alloc_strat !filename !filename
  with
  | Error (errs, trace) ->
      if !show_trace then
