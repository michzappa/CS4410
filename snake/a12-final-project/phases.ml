...
type phase =
  ...
  | DesugaredInterpolations of sourcespan program
  | WellFormed of sourcespan program
  | Renamed of tag program
  | Untagged of unit program
  | Desugared of unit program
  | AddedNatives of unit program
  | Tagged of tag program
  | ANFed of tag aprogram
  | Lifted of tag aprogram
  | DCEed of tag aprogram
...
let untagged p = Untagged p

let desugared p = Desugared p

let desugared_interpolations p = DesugaredInterpolations p
...
let add_err_phase
    (log : 'b -> phase)
    (next : 'a -> 'b fallible)
    (cur_pipeline : 'a pipeline) : 'b pipeline =
  ...
  | Ok (cur_val, trace) -> (
    ...
    | err ->
        Error
          ( [Failure ("Unexpected compile error: " ^ Printexc.to_string err)],
            trace ) )
;;
...
let print_trace (trace : phase list) : string list =
  let phase_name p =
    ...
    | Source _ -> "Source"
    | Parsed _ -> "Parsed"
    | DesugaredInterpolations _ -> "DesugaredInterpolations"
  ...
  let string_of_phase p =
    match p with
    | Source s -> s
    | Parsed p | WellFormed p | DesugaredInterpolations p -> string_of_program p
    | Renamed p -> string_of_program p
    | Untagged p | Desugared p | AddedNatives p -> string_of_program p
    | Tagged p -> string_of_program_with 1000 (fun tag -> sprintf "@%d" tag) p
    | ANFed p | Lifted p | DCEed p ->
        ...
        ^ ExtString.String.join "\n"
            (List.map
               (fun (tag, env) ->
                 ...
                 ^ ExtString.String.join "\n\t"
                     (List.map
                        (fun (name, arg) -> name ^ "=>" ^ arg_to_asm arg)
                        env ) )
               e )
    | Result s -> s
  in
