...
let string_of_eop2 (op : eagerprim2) =
  match op with
  | Plus -> "+"
  | Minus -> "-"
  | Times -> "*"
...
let string_of_lop2 (op : lazyprim2) =
  match op with
  | And -> "&&"
  | Or -> "||"
;;
...
let string_of_op2 (op : prim2) =
  match op with
  | EP2 sop -> string_of_eop2 sop
  | LP2 sop -> string_of_lop2 sop
;;

let name_of_eop2 (op : eagerprim2) =
  match op with
  | Plus -> "Plus"
  | Minus -> "Minus"
  | Times -> "Times"
...
let rec string_of_bind (b : 'a bind) : string =
  ...
  | BName (name, _) -> name
  | BTuple (binds, _) ->
      "(" ^ ExtString.String.join ", " (List.map string_of_bind binds) ^ ")"
;;

let rec string_of_binding_with
    (depth : int)
    (print_a : 'a -> string)
    ((bind, expr, _) : 'a binding) : string =
  sprintf "%s = %s" (string_of_bind bind)
    (string_of_expr_with depth print_a expr)

and string_of_binding (b : 'a binding) : string =
  string_of_binding_with 1000 (fun _ -> "") b

and string_of_expr_with (depth : int) (print_a : 'a -> string) (e : 'a expr) :
    string =
  ...
  if depth <= 0 then
    ...
    | EStrConcat (lhs, rhs, a) ->
        sprintf "(%s ^ %s)%s" (string_of_expr lhs) (string_of_expr rhs)
          (print_a a)
    | EStrSlice (str, low, high, a) ->
        sprintf "%s[%s..%s]%s" (string_of_expr str) (string_of_expr low)
          (string_of_expr high) (print_a a)
    | EStrSliceNoHigh (str, low, a) ->
        sprintf "%s[%s..%s]%s" (string_of_expr str) (string_of_expr low)
          "end_of_string" (print_a a)
    | EGetItem (e, idx, a) ->
        sprintf "%s[%s]%s" (string_of_expr e) (string_of_expr idx) (print_a a)
    | ESetItem (e, idx, newval, a) ->
        sprintf "%s[%s] := %s %s" (string_of_expr e) (string_of_expr idx)
          (string_of_expr newval) (print_a a)
    | ENumber (n, a) -> Int64.to_string n ^ print_a a
    | EBool (b, a) -> string_of_bool b ^ print_a a
    | ENil a -> "nil " ^ print_a a
    | EId (x, a) -> x ^ print_a a
    | EString (x, a) -> quote (String.escaped x) ^ print_a a
    | EPrim1 (op, e, a) ->
    ...
    | ELet (binds, body, a) ->
        let binds_strs =
          List.map (string_of_binding_with (depth - 1) print_a) binds
        in
        let binds_str = List.fold_left ( ^ ) "" (intersperse binds_strs ", ") in
        sprintf "(let %s in %s)%s" binds_str (string_of_expr body) (print_a a)
    | EIf (cond, thn, els, a) ->
        sprintf "(if %s: %s else: %s)%s" (string_of_expr cond)
          (string_of_expr thn) (string_of_expr els) (print_a a)
    | ENativeApp (fname, args, a) ->
        sprintf "(native$%s(%s))%s" fname
          (ExtString.String.join ", " (List.map string_of_expr args))
          (print_a a)
    | EApp (func, args, a) ->
        sprintf "(%s(%s))%s" (string_of_expr func)
          (ExtString.String.join ", " (List.map string_of_expr args))
          (print_a a)
    | ELetRec (binds, body, a) ->
        ...
        in
        let binds_str = List.fold_left ( ^ ) "" (intersperse binds_strs ", ") in
        sprintf "(let-rec %s in %s)%s" binds_str (string_of_expr body)
          (print_a a)
...
let string_of_decl_with (depth : int) (print_a : 'a -> string) (d : 'a decl) :
    string =
  match d with
  ...
  | Program (decls, body, a) ->
      let help group =
        ExtString.String.join "\nand "
          (List.map (string_of_decl_with depth print_a) group)
      in
      ExtString.String.join "\n\n" (List.map help decls)
      ^ "\n"
...
let string_of_ro_data (ro : read_only_data) : string =
  match ro with
  | String s -> s
  | EmptyTuple -> "()"
  | EmptyFvLambda arg_cnt -> sprintf "lambda(%d)" arg_cnt
;;

let rec string_of_aexpr_with
    (depth : int)
    (print_a : 'a -> string)
    (e : 'a aexpr) : string =
  ...
  if depth <= 0 then
    ...
    match e with
    | ASeq (e1, e2, a) ->
        sprintf "(%s ; %s)%s" (string_of_cexpr e1) (string_of_aexpr e2)
          (print_a a)
    | ALet (x, e, b, a) ->
        sprintf "(alet %s = %s in %s)%s" x (string_of_cexpr e)
...
and string_of_cexpr_with (depth : int) (print_a : 'a -> string) (c : 'a cexpr) :
    string =
  let string_of_aexpr = string_of_aexpr_with (depth - 1) print_a in
  ...
  if depth <= 0 then
    ...
    | CTuple (imms, a) ->
        sprintf "(%s)%s"
          (ExtString.String.join ", " (List.map string_of_immexpr imms))
          (print_a a)
    | CGetItem (e, idx, a) ->
        sprintf "%s[%s]%s" (string_of_immexpr e) (string_of_immexpr idx)
          (print_a a)
    ...
    | CEPrim2 (op, left, right, a) ->
        sprintf "(%s %s %s)%s" (string_of_immexpr left) (string_of_eop2 op)
          (string_of_immexpr right) (print_a a)
    | CLPrim2 (op, left, right, a) ->
        sprintf "(%s %s %s)%s" (string_of_immexpr left) (string_of_lop2 op)
          (string_of_aexpr right) (print_a a)
    | CIf (cond, thn, els, a) ->
        sprintf "(if %s: %s else: %s)%s" (string_of_immexpr cond)
          (string_of_aexpr thn) (string_of_aexpr els) (print_a a)
    | CNativeApp (fname, args, a) ->
        sprintf "(native$%s(%s))%s" fname
          (ExtString.String.join ", " (List.map string_of_immexpr args))
          (print_a a)
    | CApp (func, args, a) ->
        sprintf "(%s(%s))%s" (string_of_immexpr func)
          (ExtString.String.join ", " (List.map string_of_immexpr args))
          (print_a a)
    | CROLambda (_, args, body, a) ->
        sprintf "(rolam(%s) %s)%s"
          (ExtString.String.join ", " args)
          (string_of_aexpr body) (print_a a)
    | CLambda (args, body, _, a) ->
        sprintf "(lam(%s) %s)%s"
          (ExtString.String.join ", " args)
          (string_of_aexpr body) (print_a a)
...
and string_of_immexpr_with (print_a : 'a -> string) (i : 'a immexpr) : string =
  ...
  | ImmString (s, a) -> sprintf "%s" s ^ print_a a
  | ImmROId (x, _, a) -> x ^ print_a a

and string_of_aprogram_with
    (depth : int)
    (print_a : 'a -> string)
    (p : 'a aprogram) : string =
  match p with
  | AProgram (body, read_onlys, a) ->
      let s_ro_data =
        List.fold_right
          (fun (ro, id) str ->
            str ^ sprintf "%s: %s\n" id (string_of_ro_data ro) )
          read_onlys ""
      in
      ( if List.length read_onlys > 0 then
          "Read-only Data:\n" ^ s_ro_data
      else
        "" )
...
let string_of_immexpr (i : 'a immexpr) : string =
  string_of_immexpr_with (fun _ -> "") i
;;
...
let rec format_expr
  ...
  | ETuple (es, a) ->
      ...
      close_paren fmt
  | EStrConcat (sl, sr, a) ->
      open_label fmt "EStrConcat" (print_a a);
      help sl;
      print_comma_sep fmt;
      help sr;
      close_paren fmt
  | EStrSlice (str, low, high, a) ->
      open_label fmt "EStrSlice" (print_a a);
      help str;
      print_comma_sep fmt;
      help low;
      print_comma_sep fmt;
  ...
  | EString (x, a) ->
      ...
      pp_print_string fmt (quote (String.escaped x));
      close_paren fmt
  | EPrim1 (op, e, a) ->
  ...
  | EIf (cond, thn, els, a) ->
      ...
      help els;
      close_paren fmt
  | ENativeApp (name, args, a) ->
      open_label fmt "ENativeApp" (print_a a);
...
let format_decl (fmt : Format.formatter) (print_a : 'a -> string) (d : 'a decl)
    : unit =
  match d with
  | DFun (name, args, body, a) ->
