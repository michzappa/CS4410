...
let parse (name : string) lexbuf : sourcespan program =
  ...
  | Parsing.Parse_error ->
      ...
      raise
        (ParseError
           (sprintf "Parse error at line %d, col %d: token `%s`" line cnum tok)
        )
;;
...
let compile_string_to_string
  ...
  |> add_err_phase parsed (fun input ->
         try Ok (parse_string name input) with err -> Error [err] )
  |> compile_to_string ~no_builtins ~no_dce alloc_strat
;;
...
let compile_file_to_string
  ...
  compile_string_to_string ~no_builtins ~no_dce alloc_strat name
    (string_of_file input_file)
;;
...
let make_tmpfiles (name : string) (std_input : string) =
  ...
  ignore
    (Unix.write_substring stdin_write std_input 0 (String.length std_input));
...
let run_no_vg (program_name : string) args std_input : (string, string) result =
  ...
  let result =
    ...
    | WEXITED n ->
        Error (sprintf "Error %d: %s" n (string_of_file rstderr_name))
    | WSIGNALED n ->
        Error (sprintf "Signalled with %d while running %s." n program_name)
    | WSTOPPED n ->
        Error
          (sprintf "Stopped with signal %d while running %s." n program_name)
  in
...
let run_asm
  ...
  let try_running =
    ...
    | WSTOPPED n ->
        Error (sprintf "Stopped with signal %d while building %s." n out)
  in
  let result =
...
let run p out runner no_builtins no_dce args std_input alloc_strat =
  let maybe_asm_string =
    compile_to_string ~no_builtins ~no_dce alloc_strat (Ok (p, []))
  in
...
let run_anf p out runner args std_input =
  let maybe_asm_string =
    ...
    | err ->
        Error [Failure ("Unexpected compile error: " ^ Printexc.to_string err)]
  in
  match maybe_asm_string with
...
type compile_opts =
  { valgrind: bool;
    ...
    string_size: int option;
    alloc_strat: alloc_strategy option }
...
let read_options filename : compile_opts =
  ...
  let heap_size =
    ...
    | None -> None
    | Some str -> Some (Scanf.sscanf str "heap %d" identity)
  in
  let string_size =
    match List.find_opt (starts_with "string ") opts with
    | None -> None
    | Some str -> Some (Scanf.sscanf str "string %d" identity)
  in
  let alloc_strat =
    match List.find_opt (starts_with "alloc ") opts with
    | None -> None
    | Some "alloc naive" -> Some Naive
    | Some "alloc register" -> Some Register
    | Some s ->
        raise
          (InternalCompilerError
             (sprintf
                "'%s' is not a valid allocation strategy, use naive or register"
                s ) )
  in
  { valgrind= List.mem "valgrind" opts;
    no_builtins= List.mem "no_builtins" opts;
    no_dce= List.mem "no_dce" opts;
    heap_size;
    string_size;
    alloc_strat }
;;
...
let parse_args (argsfile : string) (opts : compile_opts) : string list =
  if Sys.file_exists argsfile then
    ...
    match (opts.heap_size, opts.string_size) with
    | Some size, Some strs -> [string_of_int size; string_of_int strs]
    | Some size, None -> [string_of_int size]
    | None, Some strs -> ["def"; string_of_int strs]
    | None, None -> []
;;
...
let test_run
    ...
    _test_ctxt =
  let full_outfile =
    "output/" ^ outfile ^ "__" ^ string_of_alloc_strategy alloc_strat
  in
  let result =
    try
      let program = parse_string outfile program_str in
      run program full_outfile run_no_vg no_builtins no_dce args std_input
        alloc_strat
...
let test_run_valgrind
  ...
  let result =
    try
      let program = parse_string outfile program_str in
      run program full_outfile run_vg no_builtins no_dce args std_input
        alloc_strat
    with err -> Error (Printexc.to_string err)
  in
  assert_equal (Ok (expected ^ "\n")) result ~cmp ~printer:result_printer
...
let test_err
  ...
  let result =
    try
      let program = parse_string outfile program_str in
      run program full_outfile runner no_builtins no_dce args std_input
        alloc_strat
    with err -> Error (Printexc.to_string err)
  in
  assert_equal (Error errmsg) result ~printer:result_printer
...
let test_over_alloc_strats make_run alloc_strat =
  match alloc_strat with
  | None -> [make_run Naive; make_run Register]
  | Some Naive -> [make_run Naive]
  | Some Register -> [make_run Register]
;;

let test_does_run filename =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/do_pass/%s.snake" filename in
  ...
  let output =
    if Sys.file_exists outfile then
      match chomp (string_of_file outfile) with
      | "" -> "PLEASE DO NOT PROVIDE AN EMPTY OUT FILE"
      | s -> s
    else
      ""
  in
  ...
  let make_run strat test_ctxt =
    runner ~no_builtins:opts.no_builtins ~no_dce:opts.no_dce ~args
      ~std_input:input strat prog ("do_pass/" ^ filename) output test_ctxt
      ~cmp:(fun check result ->
        match (check, result) with
        | Ok expect_msg, Ok actual_message ->
            String.exists actual_message expect_msg
        | _ -> false )
  in
  test_over_alloc_strats make_run alloc_strat
;;

let test_does_err filename =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/do_err/%s.snake" filename in
  ...
  let err =
    if Sys.file_exists errfile then
      match chomp (string_of_file errfile) with
      | "" -> "Please do not provide an empty error file"
      | s -> s
    else
      "Please provide an error file or move the test"
  in
  let input = if Sys.file_exists infile then string_of_file infile else "" in
  let alloc_strat = opts.alloc_strat in
  let make_run strat test_ctxt =
    test_err ~no_builtins:opts.no_builtins ~no_dce:opts.no_dce ~args
      ~std_input:input strat prog ("do_err/" ^ filename) err ~vg:opts.valgrind
      test_ctxt
  in
  test_over_alloc_strats make_run alloc_strat
;;
...
let test_doesnt_run filename _test_ctxt =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/dont_pass/%s.snake" filename in
  ...
  let result =
    try
      ...
      run program full_outfile runner opts.no_builtins opts.no_dce args input
        (* We don't use the "dont_*" tests so unwrapping to pass type check
           here. *)
        (Option.get alloc_strat)
    with err -> Error (Printexc.to_string err)
  in
  match result with
  | Ok unexpected ->
  ...
  | Error _ ->
      assert_bool
        (sprintf "Program %s currently fails (as expected for now)" filename)
        true
;;

let test_doesnt_err filename _test_ctxt =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "input/dont_err/%s.snake" filename in
  ...
  | Ok _ ->
      assert_bool
        (sprintf "Program %s currently runs (as expected for now)" filename)
        true
  | Error errmsg ->
      assert_failure
...
let input_file_test_suite () =
  let safe_readdir dir ext =
    try
      List.filter
        (fun f -> Filename.check_suffix f ext)
  ...
  >::: [ "do_pass"
         >::: flat_map
                (fun f -> List.map (fun t -> f >:: t) (test_does_run f))
                (safe_readdir "input/do_pass" ".snake");
         "do_err"
         >::: flat_map
                (fun f -> List.map (fun t -> f >:: t) (test_does_err f))
                (safe_readdir "input/do_err" ".snake");
         "dont_pass"
         >::: List.map
                (fun f -> f >:: test_doesnt_run f)
                (safe_readdir "input/dont_pass" ".snake");
         "dont_err"
         >::: List.map
                (fun f -> f >:: test_doesnt_err f)
                (safe_readdir "input/dont_err" ".snake") ]
;;
