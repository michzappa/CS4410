...
let is_well_formed (p : sourcespan program) : sourcespan program fallible =
  ...
  and wfE (name_list : (string * sourcespan) list) (e : sourcespan expr) :
    ...
    | ENil _ -> []
    (* Strings are inherently well-formed *)
    | EString _ -> []
    ...
    (* Native apps are only generated by the compile so no need to check name *)
    | ENativeApp (_, args, _) -> flat_map wfEsub args
    ...
    | EStrSlice (s, l, h, _) -> wfEsub s @ wfEsub l @ wfEsub h
    | EStrSliceNoHigh (s, l, _) -> wfEsub s @ wfEsub l
    | EStrConcat (sl, sr, _) -> wfEsub sl @ wfEsub sr
